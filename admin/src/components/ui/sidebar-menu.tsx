...
function SidebarMenuItem({ className, item, depth = 0 }: any) {
  ...
  const hasChildren = Array.isArray(items) && items.length;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <RenderComponent conditional={!!hasChildren}>
            <span>{!isOpen ? <ExpandLessIcon /> : <ExpandMoreIcon />}</span>
          </RenderComponent>
        </button>
      </motion.li>

      {/* AnimatePresence requires a direct ternary — RenderComponent stays mounted
          and prevents Framer Motion from detecting child unmount for exit animations */}
...
export function SidebarMenu({ items, className }: any) {
  return (
